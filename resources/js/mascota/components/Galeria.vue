...
const guardarGaleria = async () => {
    ...
    formData.append('mascota_id', mascota.value.id)

    try {
        guardandoGaleria.value = true
        const response = await axios.post(baseUrl + '/api/mascota-galeria', formData)
        refForm.value.reset()
        getGaleria()
    } catch (error) {
        Swal.fire({
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                const response = await axios.delete(baseUrl + '/api/mascota-galeria/' + imagen.id)
                if(response.data){
                    getGaleria()
                }
            } catch (error) {
...
const getGaleria = async () => {
    try {
        const response = await axios.get(baseUrl + '/api/mascota-galeria',{
            params: {
